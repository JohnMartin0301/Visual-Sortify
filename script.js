    ...
    for (let j = 0; j < array.length - i - 1; j++) {
      if (array[j] > array[j + 1]) {
        for (let k = 0; k < bars.length; k++) {
          if (k !== j && k !== j + 1) {
            bars[k].style.backgroundColor = "#1da1f2";
          }
        }
        let temp = array[j];
        array[j] = array[j + 1];
        ...
        bars[j].style.backgroundColor = "#39FF14";
        bars[j + 1].style.height = array[j + 1] * heightFactor + "px";
        bars[j + 1].style.backgroundColor = "#39FF14";
...
async function swap(items, leftIndex, rightIndex, bars) {
  ...
  bars[leftIndex].style.backgroundColor = "#39FF14";
  bars[rightIndex].style.height = items[rightIndex] * heightFactor + "px";
  bars[rightIndex].style.backgroundColor = "#39FF14";
  await sleep(speedFactor);
}
async function partition(items, left, right) {
  ...
  bars[pivotIndex].style.backgroundColor = "#39FF14";
...
async function quickSort(items, left, right) {
  ...
  for (let i = 0; i < bars.length; i++) {
    bars[i].style.backgroundColor = "#1da1f2";
  }
  return items;
}
